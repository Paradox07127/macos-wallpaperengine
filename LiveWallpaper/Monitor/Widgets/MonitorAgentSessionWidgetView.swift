...
struct MonitorAgentSessionWidgetView: View {
...
private struct AgentSessionCompactRow: View {
    ...
    private var accentColor: Color { MonitorAgentSessionWidgetView.accentColor(status) }
    ...
    var body: some View {
        HStack(spacing: scale.label * 0.5) {
            ...
            Text(verbatim: session.projectName)
                ...
                .truncationMode(.tail)
                .layoutPriority(1)
            if let scope = MonitorAgentSessionWidgetView.scopeLabel(for: session) {
                Text(verbatim: scope)
                    .font(MonitorDesign.captionFont(size: scale.body))
                    .foregroundStyle(MonitorDesign.inkFaint)
                    .lineLimit(1)
            ...
            if isLive, MonitorAgentSessionWidgetView.warningLabel(for: session) != nil {
                Circle()
                    .fill(MonitorDesign.signalCoral)
                    .frame(width: scale.label * 0.46, height: scale.label * 0.46)
                    .shadow(color: MonitorDesign.signalCoral.opacity(0.6), radius: 3)
...
private struct AgentSessionFullRow: View {
    ...
    private var header: some View {
        HStack(spacing: scale.label * 0.5) {
            ...
            Text(verbatim: session.projectName)
                ...
                .layoutPriority(1)
            Spacer(minLength: scale.label * 0.3)
            if let warn = MonitorAgentSessionWidgetView.warningLabel(for: session) {
    ...
    private var secondTier: some View {
        let scope = MonitorAgentSessionWidgetView.scopeLabel(for: session)
        ...
        if scope != nil || session.model != nil || !(detail ?? "").isEmpty {
            HStack(spacing: scale.label * 0.45) {
                ...
                if let tokens = MonitorAgentSessionWidgetView.tokenText(for: session) {
...
private struct AgentSessionRowTimer: View {
    ...
    var body: some View {
        if let timer = MonitorAgentSessionWidgetView.timerText(for: session, now: now) {
            ...
            Text(MonitorAgentSessionWidgetView.statusWord(session.status))
                .font(MonitorDesign.labelFont(size: scale.label))
                .foregroundStyle(MonitorDesign.inkFaint)
                .lineLimit(1)
                .layoutPriority(1)
...
private struct AgentSessionWarningChip: View {
    let warn: MonitorAgentSessionWidgetView.WarningInfo
    let scale: AgentTypeScale
...
extension MonitorAgentSessionWidgetView {
...
#Preview("Agent Session · M") {
    TimelineView(.periodic(from: .now, by: 1)) { t in
        VStack(spacing: 20) {
            MonitorAgentSessionWidgetView(context: .agentSessionSample(size: .medium).at(t.date))
                .frame(width: 364, height: 170)
            MonitorAgentSessionWidgetView(context: .agentSessionQuiet(size: .medium).at(t.date))
                .frame(width: 364, height: 170)
        }
        .padding(32)
...
#Preview("Agent Session · L") {
    TimelineView(.periodic(from: .now, by: 1)) { t in
        MonitorAgentSessionWidgetView(context: .agentSessionSample(size: .large).at(t.date))
