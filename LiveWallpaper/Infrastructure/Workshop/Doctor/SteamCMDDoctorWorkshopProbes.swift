...
/// The Workshop-wide checks, as opposed to the SteamCMD-specific ones.
///
/// All three are advisory: `downloadBlocker` does not read them, so nothing
/// here can take a command away from the user. They exist because each one
/// fails silently in production — a stale assets bookmark simply skips layers,
/// an unreachable connector only surfaces as "the download did nothing".
extension SteamCMDDoctorService {

    // MARK: - Workshop content folder

    /// Whether the Workshop items Steam has already downloaded are reachable.
    ///
    /// Separate from `workingDirectory`, which only proves `config.vdf` is
    /// readable: a Steam profile with no Wallpaper Engine subscriptions has a
    /// perfectly good config and no content folder at all, and the difference
    /// is exactly what a user staring at an empty library needs told.
        ...
        guard workdirBookmarkData != nil else {
            setProbe(.workshopContent, status: .yellow(
                message: String(
                    ...
                    comment: "Workshop content diagnostic when the Steam library has not been authorized."
                ),
                command: nil
            ))
            return
            ...
            guard fileManager.fileExists(atPath: path, isDirectory: &isDirectory), isDirectory.boolValue else {
                setProbe(.workshopContent, status: .yellow(
                    message: String(
                        ...
                        comment: "Workshop content diagnostic when the content folder does not exist."
                    ),
                    command: nil
                ))
                return
            ...
            guard let entries = try? fileManager.contentsOfDirectory(atPath: path) else {
                setProbe(.workshopContent, status: .red(
                    message: redacted(String(
                        ...
                        comment: "Workshop content diagnostic when the folder exists but is unreadable."
                    )),
                    command: nil
                ))
                return
    ...
    /// Whether the Wallpaper Engine shared assets are actually reachable.
    ///
    /// A bookmark that no longer resolves is the failure worth catching: scenes
    /// still render, they just quietly drop every layer that referenced the
    /// shared assets, so nothing else in the app reports it.
    func runSceneResourcesProbe() {
        ...
        guard library.isAuthorized || WPEEngineAssetsLibrary.managedInstallRoot() != nil else {
            setProbe(.sceneResources, status: .yellow(
                message: String(
                    ...
                    comment: "Scene resources diagnostic when no Wallpaper Engine install is linked."
                ),
                command: nil
            ))
            return
        ...
        guard let root = WPEEngineAssetsLibrary.managedInstallRoot() ?? library.resolveAuthorizedRoot() else {
            setProbe(.sceneResources, status: .red(
                message: String(
                    ...
                    comment: "Scene resources diagnostic when the stored bookmark no longer resolves."
                ),
                command: nil
            ))
            return
        ...
        guard fileManager.fileExists(atPath: root.path(percentEncoded: false)) else {
            setProbe(.sceneResources, status: .red(
                message: redacted(String(
                    ...
                    comment: "Scene resources diagnostic when the linked folder no longer exists."
                )),
                command: nil
            ))
            return
        ...
        setProbe(.sceneResources, status: .green(detail: redacted(String(
            ...
            comment: "Scene resources diagnostic detail when the assets folder is reachable."
        ))))
    }

    // MARK: - Connector

    /// Whether the unsandboxed helper answers at all.
    ///
    /// `locateSteamCMDBinary` returns nil only when the XPC connection fails —
    /// a successful call that finds nothing still decodes a result — so it
    /// separates "the helper is not running" from "there is no SteamCMD",
    /// which every other failure in this list confuses.
    func runConnectorProbe() async {
        guard await SteamConnectorClient.locateSteamCMDBinary() != nil else {
            setProbe(.connector, status: .red(
                message: String(
                    ...
                    comment: "Connector diagnostic when the XPC service is unreachable."
                ),
                command: nil
            ))
            return
        ...
        setProbe(.connector, status: .green(detail: String(
            ...
            comment: "Connector diagnostic detail when the XPC service replies."
        )))
    }
}
#endif
