...
///
/// The password field's contents go to the connector once per attempt and end
/// at steamcmd's own prompt; the sheet keeps them only in `@State` for the
/// lifetime of the attempt (a Guard retry reuses them, which is why they are
/// not cleared between rounds) and the window's teardown drops them. Nothing
/// here or downstream writes them anywhere.
struct SteamSignInSheet: View {
    ...
    private func handle(_ result: SteamCMDLoginResult?) {
        ...
        case .invalidPassword:
            phase = .form
            errorText = String(localized: "Steam rejected that account name or password.", comment: "In-app Steam sign-in failure.")
        case .invalidGuardCode:
            phase = .guardCode(email: true)
            errorText = String(localized: "That Steam Guard code wasn't accepted.", comment: "In-app Steam sign-in failure.")
        case .rateLimited:
            phase = .form
            errorText = String(localized: "Steam is rate-limiting sign-ins from this Mac. Wait a few minutes and try again.", comment: "In-app Steam sign-in failure.")
        case .timedOut:
            phase = .form
            errorText = String(localized: "Steam didn't confirm the sign-in in time. If Steam Guard asked on your phone, approve it and try again.", comment: "In-app Steam sign-in failure.")
        case .failed, .unavailable, nil:
            phase = .form
            errorText = String(localized: "Sign-in didn't complete. Check the connection and try again.", comment: "In-app Steam sign-in failure.")
        }
    }
}
#endif
