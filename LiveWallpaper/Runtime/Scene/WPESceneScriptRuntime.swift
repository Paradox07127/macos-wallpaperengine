...
final class WPEDynamicTransformScriptInstance: @unchecked Sendable {
    ...
    func tick(
        ...
        guard !isPoisoned, !isDestroyed, !engine.hasRuntimeFault, engine.allows(.tick) else { return nil }
        switch engine.tick(
    ...
    func seedAsyncTick(pointerPosition: SIMD2<Double>, runtimeSeconds: Double? = nil) {
        guard !isPoisoned, !isDestroyed, !engine.hasRuntimeFault, engine.allows(.tick) else { return }
        switch engine.tick(
            currentValue: lastValue,
            pointerPosition: pointerPosition,
            runtimeSeconds: runtimeSeconds,
