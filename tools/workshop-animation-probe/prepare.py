...
for filename,path in [('AnimatedGIFThumbnail.swift','LiveWallpaper/Views/Workshop/AnimatedGIFThumbnail.swift'),('ScenePreview.swift','LiveWallpaper/Views/ScreenDetail/ScenePreview.swift')]:
 ...
 if filename=='AnimatedGIFThumbnail.swift':
  needle='if let frame { self.displayedFrame = frame }';assert needle in s;s=s.replace(needle,'if let frame { AnimationMetrics.frames += 1; AnimationMetrics.frameTimes[String(describing: ObjectIdentifier(self)), default: []].append(ProcessInfo.processInfo.systemUptime); self.displayedFrame = frame }')
 else:
  needle='if let frame { self.layer?.contents = frame }';assert needle in s;s=s.replace(needle,'if let frame { AnimationMetrics.frames += 1; AnimationMetrics.frameTimes[String(describing: ObjectIdentifier(self)), default: []].append(ProcessInfo.processInfo.systemUptime); self.layer?.contents = frame }')
