...
- **设置窗口**：显示器、**已保存**（壁纸与整屏方案）、Apple 航拍、Steam 创意工坊（Pro），以及系统壁纸（macOS 26+）。
- **显示器检查器**：壁纸与叠加层两个标签；叠加层内分别提供天气、小组件、音乐页面和各自的预览。
- **语言**：英语、简体中文、繁体中文、日语、西班牙语。

| 设置页 | 版本 | 内容 |
|---|---|---|
| 通用 | 两者 | 语言、外观、卡片大小、登录启动、自动检查更新、Dock、锁屏截帧、截屏与共享中是否显示壁纸 |
| 显示器默认值 | 两者 | 显示器排列与重命名；播放、帧率、适配、色彩和交互默认值 |
| 快捷键 | 两者 | 总开关与八个可绑定动作 |
| 性能 | 两者 | 暂停规则、应用例外、视频预载；Pro 另有场景自适应帧率、渲染线程，硬件支持时还有 MetalFX 超分辨率与 HDR 输出 |
| 系统联动 | 两者 | 音频响应（Pro）：为场景和音乐视觉效果捕获系统音频；天气：关闭、系统定位或手动位置 |
| 叠加层 | 两者 | 所有小组件面板的配色、不透明度与液态玻璃（macOS 26+）；温度单位 |
| 系统壁纸 | 两者，macOS 26+ | 视频播放方式、扩展状态、服务维护、从系统壁纸库移除全部视频 |
| 创意工坊 | Pro | Steam 配置、API key、引擎资源、浏览偏好、缩略图标签、诊断、隐私与条款 |
| 存储 | Pro | 已下载项目、引擎资源、系统壁纸视频副本、缓存 |
| 备份与恢复 | 两者 | 导出与导入配置 |
| 高级 | 两者 | 诊断摘要与导出、报告问题、日志文件、重置所有设置 |
| 关于 | 两者 | 版本、更新、GitHub 与讨论区链接、报告问题、欢迎导览 |
...
浏览、搜索并应用 macOS 已下载的航拍视频。航拍壁纸走视频播放路径，Loomscreen 不负责下载 Apple 目录。
...
- **时钟**：独立的透明辉光管时钟，带金属底座与细玻璃管冒号。支持拖动位置、自由缩放、12/24 小时制、小时首位补零、透明度和冒号闪动；拥有独立的逐屏开关与图层，被遮挡时暂停，且不启动资源采样。
