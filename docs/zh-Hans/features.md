...
- **监控面板**：系统总览、辉光管时钟、CPU、内存、GPU、网络、磁盘、电源、进程、Agent 会话、ANE 内存、天气，共十二种组件。按类型提供小/中/大尺寸、拖拽排列、显示缩放、选项和布局导入/导出。可位于桌面或窗口上方。缺测与零读数有区别，曲线保留采样空隙；网络/磁盘累计为本次监控估算。
- **系统总览**：用一个中型或大型仪表整合 CPU、内存、GPU、网络吞吐、磁盘 I/O 和电源。大型增加历史曲线以及可选的温度、风扇读数，复用现有共享采样管线。
- **辉光管时钟**：六支玻璃管在透明背景上显示本地时间，复用面板的共享时钟和暂停机制，支持中、大尺寸。
