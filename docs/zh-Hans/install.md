...
两个版本都会在启动时、以及每次打开菜单栏弹窗时查一次 GitHub Releases API，并以 12
小时节流——没有后台轮询。有新版本时，两个地方会提示，打开的是同一个 release 页面：

- 菜单栏弹窗里、总开关左边的 **Update** 按钮；
- **设置 → 关于** 里的横幅，你也可以在那里**立即检查**或**跳过此版本**。

在那里跳过某个版本，两处会一起不再提示——它们读的是同一次检查。Lite 和 Pro 发在同一个
release 里，所以落地的那个页面同时挂着两个 DMG，由你挑自己在用的那个。
