...
struct NetworkWidgetTests {
    ...
    func activeNilForEmpty() {
        #expect(NetworkWidgetView.pickActiveInterface(nil) == nil)
        #expect(NetworkWidgetView.pickActiveInterface([]) == nil)
    }

    @Test("the NWPath-chosen isActive interface wins over higher traffic")
    func activePrefersFlag() {
        let picked = NetworkWidgetView.pickActiveInterface([
    ...
    func ipv4Discriminator() {
        #expect(NetworkWidgetView.isIPv4("192.168.1.24"))
        #expect(NetworkWidgetView.isIPv4("fe80::14b2:9c3f:8e1a:22d7") == false)
        #expect(NetworkWidgetView.isIPv4("::1") == false)
    }

    @Test("rate splits into numeral and unit at the first space")
    func rateSplit() {
        let mb = NetworkWidgetView.splitRate("6.2 MB/s")
        #expect(mb.value == "6.2")
        #expect(mb.unit == "MB/s")
        #expect(NetworkWidgetView.splitRate("0 B/s").unit == "B/s")
