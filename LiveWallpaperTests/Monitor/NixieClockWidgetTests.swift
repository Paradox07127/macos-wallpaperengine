...
final class NixieClockWidgetTests: XCTestCase {
    ...
    func testTimeRollsOverWithoutLosingLeadingZeroes() {
        XCTAssertEqual(NixieClockWidgetView.digits(at: Date(timeIntervalSince1970: 0), calendar: calendar), [0, 0, 0, 0, 0, 0])
        XCTAssertEqual(NixieClockWidgetView.digits(at: Date(timeIntervalSince1970: 86399), calendar: calendar), [2, 3, 5, 9, 5, 9])
        XCTAssertEqual(NixieClockWidgetView.digits(at: Date(timeIntervalSince1970: 86400), calendar: calendar), [0, 0, 0, 0, 0, 0])
    }
    ...
    func testFitsBothBoardSizesWithoutStretchingTubes() {
        for bounds in [CGSize(width: 356, height: 170), CGSize(width: 356, height: 356)] {
            let fitted = NixieClockWidgetView.fittedSize(in: bounds)
    ...
    func testNativeTransparentCompositionAndCapture() throws {
        ...
        let root = WidgetFactory.tile(context: context()).frame(width: dimensions.width, height: dimensions.height)
