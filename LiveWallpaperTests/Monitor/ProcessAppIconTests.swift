...
struct ProcessAppIconTests {
    ...
    func renderProcesses() throws {
        ...
        let samples = [
            MonitorProcessSample(name: "Finder", cpuPercent: 12, memBytes: 100_000_000, bundleID: "com.apple.finder"),
            MonitorProcessSample(name: "kernel_task", cpuPercent: 10, memBytes: 200_000_000),
            MonitorProcessSample(name: "Terminal", cpuPercent: 5, memBytes: 30_000_000, bundleID: "com.apple.Terminal"),
            MonitorProcessSample(name: "node", cpuPercent: 2, memBytes: 20_000_000),
        ]
        let context = MonitorWidgetContext(snapshot: MonitorSnapshot(system: MonitorSystemSnapshot(topProcesses: samples)),
                                           history: MonitorHistorySnapshot(), placement: MonitorWidgetPlacement(kind: .processes, size: .medium),
                                           isEditing: false, reduceMotion: true, now: Date())
        let content = ProcessesWidgetView(context: context).frame(width: 356, height: 170)
        ...
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("process-widget-icons.png")
