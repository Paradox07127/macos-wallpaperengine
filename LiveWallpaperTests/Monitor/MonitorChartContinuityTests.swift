...
/// Charts break the path at a gap rather than interpolating across it. The tolerance is
/// sized from the drawn window's own cadence, not the whole buffer's median — a buffer
/// median outlives a cadence change and shreds the window.
@Suite("Monitor chart continuity")
struct MonitorChartContinuityTests {
    /// Steps off the real sampler at a nominal 1 s cadence: the poll loop sleeps a fixed
    /// interval after its work, so every step runs long.
...
/// A sample is a measurement, so the axis records the time the measurement was taken,
/// never the publish clock and never `Date()`.
